...
### Added
- **`codex` skill** — lets Claude use OpenAI Codex as an autonomous peer rather than a tool: delegate a self-contained task, argue a design across multiple turns, or fan out parallel sub-agents via `ay codex`. It never prompts for a model or reasoning effort, so it works while nobody is at the keyboard. Everything runs through `scripts/codex-run.sh` (read-only by default, `--write` to allow edits, `--resume <THREAD_ID>` to continue an exact conversation, `--dir` to target another path); the answer goes to stdout and the thread id to stderr.
- **Model choice is a rule, not a constant.** The wrapper passes no `-m`/`-c`, so Codex inherits `~/.codex/config.toml` and a stronger model is adopted by editing that config with nothing to change in the skill. A hardcoded name was the first draft and was wrong for a measurable reason: the upstream skill this replaces shipped a "complete" model list that already omitted `gpt-5.6-pro`. The skill instead documents the two rejection errors, which mean opposite things — `requires a newer version of Codex` means the **CLI** is stale (upgrade, retry the *same* model) while `not supported when using Codex with a ChatGPT account` means it is unlicensed (step down one). Reading the first as "model unavailable" silently pins you to a weaker model forever.
- **`codex-side/` — the mirror, so Codex can call Claude.** `bash codex-side/install.sh` drops a `claude` skill into `~/.codex/skills/`, giving Codex the same three modes pointed back at Claude through `claude-run.sh`. It ships outside `skills/` because Claude's plugin system only deploys Claude skills; Codex loads from its own directory and would never see it. The file is named `SKILL.codex.md` in the repo and renamed on install so Claude's skill auto-discovery doesn't try to load a Codex skill. Verified round-trip: Claude → Codex → Claude.
- The wrapper encodes six traps that each fail **silently** — empty output, exit 0, no error — so hitting one costs a turn to diagnose. `codex exec` reads stdin even with a positional prompt and hangs if it is left open (needs `</dev/null`), but `codex exec resume` takes its prompt *through* stdin, so the same redirect discards it and returns exit 0 with nothing. `--approve-for-me` refuses to combine with `--sandbox` (it already implies `workspace-write`), and `--full-auto` no longer exists at all. Thread ids must be parsed anchored on `session id:`, since the temp-file path in stderr contains a uuid that otherwise matches first and resumes the wrong conversation — or a nonexistent one. And macOS still ships bash 3.2, where `"${ARR[@]}"` on an empty array under `set -u` throws `unbound variable`; that one broke the resume path until the `${ARR[@]+...}` guard went in.
- Both skills default to the strongest model at the highest effort. Running at the ceiling is close to free on easy work — cost and latency scale with task difficulty rather than being a flat tax (a trivial prompt returned in ~5s at max effort), so there is no reason to economize by default.
...
### Added
- **`sunday-inbox` skill** — reviews Sunday-managed Gmail, tracks replies, tasks, deadlines, and scheduling, applies a strict durable-document boundary for `6 Protected`, and files temporary travel records under `7 For Reference`. When the user explicitly asks for a phone update, the skill prefers a direct texting tool and otherwise invokes Sunday's existing fixed-self Messages command through the local agent runner exactly once; recipient data remains in Sunday configuration, message text is passed as data rather than executable AppleScript, and uncertain failures are never retried automatically.
