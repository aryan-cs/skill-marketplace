...
- **`install-smart-lid.sh` could not upgrade over a running daemon.** Every `install` on a machine that already had the service loaded failed with `Could not unload the existing smart-lid service.` immediately followed by `line 87: transaction_active: unbound variable`. Two independent bugs, both reachable only on the upgrade path — which is why the first install on a clean machine succeeded and the first upgrade did not.
  - `launchctl bootout` tears the service down **asynchronously** and returns before teardown completes, so the single `service_loaded` check on the next line saw the service still present and aborted a healthy upgrade. The startup path already polls (`until service_running`) for precisely this reason; teardown now polls the same way. Confirmed on a real machine: the daemon was in fact already gone and its own exit trap had restored `SleepDisabled=0`, while the installer was reporting failure.
  - `rollback_install()` runs from the `EXIT` trap, which fires only after `install_daemon()` has returned — at which point its `local` bookkeeping variables are out of scope, so `set -u` killed the trap on its first reference. The rollback therefore never ran: it neither restored the service nor removed the transaction's temp files, orphaning `.new.<pid>` and `.backup.<pid>` copies in `/usr/local/libexec` and `/Library/LaunchDaemons`. That state is now script-scope.
- **`simulate` signalled real `caffeinate` processes on the host.** The subcommand documents itself as a test/debug path that refuses to make real power changes, and gates `apply_power_state` behind `SMART_LID_SIMULATION_APPLY` accordingly — but the `caffeinate` release added alongside the lid-open guard was called straight out of `enforce_low_battery_sleep` with no such gate. So a plain `./smart-lid-daemon.sh simulate` fed a single below-cutoff sample used the real `pgrep`/`kill` and TERMed **every** `caffeinate` on the machine, silently dropping the keep-awake holds of unrelated agent sessions. Found the direct way: it happened while verifying an install, taking the system-wide assertion count to zero. `simulate` now defaults `RELEASE_CAFFEINATE` to `0` and requires an explicit `SMART_LID_RELEASE_CAFFEINATE=1`; the simulated *policy* is still reported in full, only the host-visible side effect is suppressed. The suite never caught it because every caffeinate case injects mock `pgrep`/`kill` binaries. The `run` path is unaffected — signalling at a real cutoff is the intended behavior there.
- `tests/test-smart-lid.sh` grew from 27 to 30 cases. The existing lifecycle fake unloaded synchronously inside `bootout`, so it could not surface either installer bug; the new fake models launchd's deferred teardown. Every new case was verified to fail against the unfixed code — the two installer ones reproducing the exact error pair observed on the machine, and the `simulate` one showing `-TERM 4242` / `-TERM 4243` reaching the kill log when they should not.

### Changed
- **Released as 0.11.0 rather than left unreleased.** `claude plugin update` compares the version in `plugin.json`, so a change merged to `main` without a bump does not reach machines that already have the plugin: `update` reports *"already at the latest version"* while the copy under `~/.claude/plugins/cache/` still holds the old code. Recovering an install that has already gone stale needs `uninstall` followed by `install` — plain `update` will not do it. Verify against the file under `cache/`, not the clone under `marketplaces/`.
