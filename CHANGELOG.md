...
- `tests/test-smart-lid.sh` grew from 27 to 29 cases. The existing lifecycle fake unloaded synchronously inside `bootout`, so it could not surface either bug; the new fake models launchd's deferred teardown. Both cases were verified to fail against the pre-fix installer, reproducing the exact error pair observed on the machine.
