...
- A skill `name` must be kebab-case, ≤ 64 chars. (Claude Code does **not** reserve `claude`/`anthropic` in skill names — a tool-specific name like `setup-claude-code` is allowed. Prefer naming for the *job* anyway: `setup-agent-mac` was renamed from `setup-claude-code` once it covered Codex and the machine itself, not just one CLI.)
