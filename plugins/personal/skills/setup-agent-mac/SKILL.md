...
3. **Verify.** Run `scripts/verify.sh`. It sources a fresh shell and checks: both env vars are set, `ay`
   ...
   lock-first, simultaneous sensor changes, daemon restart, and install/uninstall behavior — and
   `tests/test-crash-dialogs.sh`, which stubs `launchctl`/`defaults` to exercise off/on/status,
   idempotency, the root guard, and the case where `launchctl disable` silently fails.
