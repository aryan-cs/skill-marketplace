...
2. **Apply the setup.** Run `scripts/setup.sh`. Idempotent (safe to re-run — every change is a marker
   ...
   - installs `agent-yes` (`npm install -g agent-yes`) if `ay` isn't on PATH, and writes the `agent-yes`
     block — a `claude()` wrapper that routes through `ay` and holds a `caffeinate` assertion so runs never idle-sleep;
